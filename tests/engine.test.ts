...
//     clothes, and a larger waterskin are merchant gear; hunted game is roasted.
{
  ...
  // roast a beast with oil (roasting requires a goblet of cooking oil)
  g.run.larder = [null, null, null, null, null, null];
  g.run.pockets = [{ item: 'raw_rabbit', qty: 1 }, { item: 'cooking_oil', qty: 1 }, null, null];
...
// 47) Roasting hunted game (with oil) cooks the beast and skins it (a skin or a hide).
{
  ...
  g.run.oilBuffMs = 3600_000; // the Chalice supplies the oil so we can roast all ten
  ...
  assert(countItem(g.run, 'raw_boar') === 0, 'all boars are roasted');
...
//     the empty bucket is handed back.
{
  ...
  dispatch(g, { type: 'doDeed', id: 'refill_well' }); // the well fills any bucket automatically
  assert(countItem(g.run, 'bucket') === 0 && countItem(g.run, 'bucket_water') === 1, 'drawing water at the well fills a bucket too');
